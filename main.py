...
async def send_reply(reply_func, args, kwargs, client: Client, chat_id: int = 0) -> None:
...
def _sync_write_file(path: str, text: str) -> None:
...
def _is_user_active(user_id: int) -> bool:
    disabled = db.get(SETTINGS_COLLECTION, "disabled_users") or []
    if user_id in disabled:
        return False
    if db.get(SETTINGS_COLLECTION, "gchat_for_all"):
        return True
    enabled = db.get(SETTINGS_COLLECTION, "enabled_users") or []
...
async def _send_bot_pics(client: Client, chat_id: int, n: int, caption: str) -> bool:
    ...
    if len(selected) > 1:
        ...
        await send_reply(client.send_media_group, [chat_id, media], {}, client, chat_id=chat_id)
    else:
        await send_reply(client.send_photo, [chat_id, selected[0]], {"caption": caption}, client, chat_id=chat_id)
    return True
...
def _should_rotate_key(exc: Exception) -> bool:
    ...
    if code in (429, 403):
        return True
    return any(k in err.lower() for k in ("429", "403", "invalid", "suspended"))
...
async def send_response_smart(
    ...
    *,
    ...
    if len(response) > RESPONSE_MAX_CHARS:
        ...
        await send_reply(message.reply_text, [response], {}, client, chat_id=chat_id)
...
async def handle_voice_message(client: Client, chat_id: int, bot_response: str) -> bool:
    ...
    if not get_voice_enabled():
        await send_reply(client.send_message, [chat_id, text], {}, client, chat_id=chat_id)
        return True
...
async def handle_gpic_message(client: Client, chat_id: int, bot_response: str) -> bool:
    ...
    if not sent:
        await send_reply(client.send_message, ["me", "No bot pictures in group/channel."], {}, client, chat_id=chat_id)
    return True
...
async def _process_sticker_gif_buffer(client: Client, user_id: int) -> None:
    try:
        ...
        await send_reply(last_msg.reply_text, [random.choice(SMILEYS)], {}, client, chat_id=last_msg.chat.id)
    except Exception as exc:
...
async def handle_files(client: Client, message: Message) -> None:
    ...
    try:
        ...
        if message.photo:
            ...
            if _image_timers.get(user_id) is None:
...
async def gchat_command(client: Client, message: Message) -> None:
    ...
    try:
        ...
        if len(parts) < 2:
            await send_reply(message.edit_text, ["Usage: gchat [on|off|del|all|r] [user_id]"], {}, client, chat_id=chat_id)
            return
        ...
        if command == "on":
            ...
            await send_reply(message.edit_text, [f"<spoiler>ON: {user_id}</spoiler>"], {}, client, chat_id=chat_id)
            ...
            if user_id in enabled_users:
                ...
                db.set(SETTINGS_COLLECTION, "enabled_users", enabled_users)
            await send_reply(message.edit_text, [f"<spoiler>OFF: {user_id}</spoiler>"], {}, client, chat_id=chat_id)

        elif command == "del":
            db.remove(HISTORY_COLLECTION, f"chat_history.{user_id}")
            await send_reply(message.edit_text, [f"<spoiler>Deleted: {user_id}</spoiler>"], {}, client, chat_id=chat_id)
            ...
            await send_reply(message.edit_text, [f"All: {status}"], {}, client, chat_id=chat_id)
            ...
            await send_reply(message.edit_text, [f"<spoiler>{label}: {user_id}</spoiler>"], {}, client, chat_id=chat_id)

        else:
            await send_reply(message.edit_text, ["Usage: gchat [on|off|del|all|r] [user_id]"], {}, client, chat_id=chat_id)

        await send_reply(message.delete, [], {}, client, chat_id=chat_id)
    except Exception as exc:
...
async def switch_role(client: Client, message: Message) -> None:
    ...
    try:
        ...
        if not roles:
            ...
            await send_reply(message.edit_text, ["Failed to fetch roles."], {}, client, chat_id=chat_id)
            return
        ...
        if len(parts) == 1:
            ...
            await send_reply(message.edit_text, [f"Roles:\n{available}"], {}, client, chat_id=chat_id)
            return

        role_name = parts[1].lower()
        if role_name in roles:
            db.set(SETTINGS_COLLECTION, f"custom_roles.{user_id}", roles[role_name])
            await send_reply(message.edit_text, [f"Switched: {role_name}"], {}, client, chat_id=chat_id)
        else:
            await send_reply(message.edit_text, [f"Not found: {role_name}"], {}, client, chat_id=chat_id)
        await send_reply(message.delete, [], {}, client, chat_id=chat_id)
    except Exception as exc:
...
async def set_custom_role(client: Client, message: Message) -> None:
    ...
    try:
        ...
        if not custom_role:
            ...
            db.remove(HISTORY_COLLECTION, f"chat_history.{user_id}")
            await send_reply(message.edit_text, [f"<spoiler>Role reset: {user_id}</spoiler>"], {}, client, chat_id=chat_id)
        else:
    ...
    try:
        ...
        if subcommand == "model":
            if key:
                ...
                await send_reply(message.edit_text, [f"Gemini model set to: {key}"], {}, client, chat_id=chat_id)
            else:
                await send_reply(message.edit_text, [f"Current Gemini model: {get_gemini_model()}"], {}, client, chat_id=chat_id)
            return

        if subcommand == "voice":
            new_state = not get_voice_enabled()
            set_voice_enabled(new_state)
            await send_reply(message.edit_text, [f"Voice: {'ON' if new_state else 'OFF'}"], {}, client, chat_id=chat_id)
            return

        if subcommand == "add" and key:
            if key in gemini_keys:
                await send_reply(message.edit_text, ["Key already added!"], {}, client, chat_id=chat_id)
                return
            gemini_keys.append(key)
            db.set(SETTINGS_COLLECTION, "gemini_keys", gemini_keys)
            await send_reply(message.edit_text, ["Gemini key added!"], {}, client, chat_id=chat_id)
            return
        ...
        if subcommand == "set" and key:
            ...
            if 0 <= index < len(gemini_keys):
                ...
                await send_reply(message.edit_text, [f"Current key set to: {key}"], {}, client, chat_id=chat_id)
            else:
                await send_reply(message.edit_text, [f"Invalid key index: {key}"], {}, client, chat_id=chat_id)
            return
        ...
        if subcommand == "del" and key:
            ...
            if 0 <= index < len(gemini_keys):
                ...
                if current_key_index >= len(gemini_keys):
                    ...
                    db.set(SETTINGS_COLLECTION, "current_key_index", current_key_index)
                await send_reply(message.edit_text, [f"Key {key} deleted!"], {}, client, chat_id=chat_id)
            else:
                await send_reply(message.edit_text, [f"Invalid key index: {key}"], {}, client, chat_id=chat_id)
            return
        ...
        if subcommand == "role":
            ...
            if key:
                ...
                if role_name in roles:
                    ...
                    await send_reply(message.edit_text, [f"Default: {role_name}"], {}, client, chat_id=chat_id)
                else:
                    await send_reply(message.edit_text, [f"Not found: {role_name}"], {}, client, chat_id=chat_id)
            else:
                roles_list = "\n".join(f"- {r}" for r in roles) if roles else "No roles found."
                await send_reply(message.edit_text, [f"Available roles:\n{roles_list}"], {}, client, chat_id=chat_id)
            return
        ...
        if subcommand == "history":
            if key and key.isdigit():
                ...
                await send_reply(message.edit_text, [f"History head/tail set to: {n}"], {}, client, chat_id=chat_id)
                return
            if len(parts) > 3 and parts[2].isdigit() and parts[3].isdigit():
                ...
                await send_reply(message.edit_text, [f"History head: {head}, tail: {tail}"], {}, client, chat_id=chat_id)
                return
        ...
        if len(menu_text) > CHUNK_SIZE:
            ...
            await send_reply(message.edit_text, [menu_text], {}, client, chat_id=chat_id)
...
async def gpic_command(client: Client, message: Message) -> None:
    ...
    try:
        ...
        sent = await _send_bot_pics(client, chat_id, n, caption)
        if not sent:
            await send_reply(message.edit_text, ["No bot pictures found."], {}, client, chat_id=chat_id)
            return
        await send_reply(message.delete, [], {}, client, chat_id=chat_id)
    except Exception as exc:
